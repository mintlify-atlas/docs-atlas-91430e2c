...
Fetches permits from the past N days by querying two DOB NOW datasets and merging the results.

```typescript src/permits.ts
...
<ParamField path="daysBack" type="number" default={30}>
  Number of days to fetch permits for. Scales API limits automatically:
  - 1 day: 1,000 permits
  - 7 days: 2,000 permits  
  - 30 days: 5,000 permits (capped to keep map usable)
</ParamField>
